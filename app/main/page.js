...
const MainMenuList = () => {
  ...
  const handleViewMainCategory = async (mainCategory) => {
    try {
      const response = await axios.get(
        `https://absback.vercel.app/api/main/${mainCategory._id}`
      );
  ...
  const handleAddSubmit = async () => {
    try {
      ...
      const response = await axios.post(
        "https://absback.vercel.app/api/main",
        formData,
        {
          headers: {
  ...
  const handleDeleteConfirmed = async (menu) => {
    try {
      ...
      const response = await axios.delete(
        `https://absback.vercel.app/api/main/${menu._id}`
      );
  ...
  const handleEditSubmit = async () => {
    try {
      ...
      const response = await axios.patch(
        `https://absback.vercel.app/api/main/${editedMenu._id}`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
      ...
      const updatedMenusResponse = await axios.get("https://absback.vercel.app/api/main");
  ...
  const handlePicturePreview = (mainImage) => {
    const completeImageUrl = `https://absback.vercel.app/${mainImage}`;
  ...
  useEffect(() => {
    const fetchMenus = async () => {
      try {
        const response = await axios.get("https://absback.vercel.app/api/main");
              ...
              <div className="p-1 text-center">
                ...
                <p>
                  ...
                  {mainCategoryToView?.mainImage ? (
                    <img
                      src={`https://absback.vercel.app/${mainCategoryToView.mainImage}`}
                      alt="Main Category"
